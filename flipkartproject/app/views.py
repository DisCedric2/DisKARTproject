...
def sortingbyprice(req):
    ...
    if sortoption == "low_to_high":
        allproducts = Product.objects.order_by("price") # ascedning order
...
def searchproduct(req):
    ...
    if query:
        ...
        if len(allproducts)==0:
            messages.error(req, "No Items Found :(")
...
def showcarts(req):
    ...
    if username.is_authenticated:
        context = {'allcarts':allcarts, "username":username.id, 'totalitems':totalitems, 'totalamount':totalamount}
    else:
...
class ProductUpdate(UpdateView):
    model = Product
    templates_name_suffix = "_update_form"
    # fields = "__all__"
